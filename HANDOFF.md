...
**วันที่:** 19 เมษายน 2026 (v2 — ADR-001 + is_first_case)
...
**หมายเหตุ:** ไฟล์ data ไม่อยู่บน GitHub (กัน file ใหญ่) — เก็บไว้บน **DagsHub** จัดการผ่าน **DVC**

### วิธีดึง data (ครั้งแรก)

```bash
# 1. Clone repo
git clone https://github.com/natchyunicorn/denttime.git
cd denttime

# 2. ติดตั้ง DVC
pip install dvc

# 3. Setup DagsHub auth (ครั้งเดียว)
#    - สมัคร DagsHub ฟรี: https://dagshub.com/user/sign_up (Sign up with GitHub)
#    - สร้าง token: https://dagshub.com/user/settings/tokens
...
| `dentist_pseudo_id` | string / null | `D_7b2c9e4f01a8d3b5` | รหัสทันตแพทย์ (HMAC) — **nullable** ตาม ADR-001 |
| `appointment_pseudo_id` | string | `A_e5d1f8a2b3c4067e` | รหัสนัดหมาย (HMAC) — unique ทุก row |
| `has_dentist_id` | int | `1` หรือ `0` | มี license_no หรือไม่ (ADR-001) — ใช้เป็น flag สำหรับ Two-Tier Lookup |
...
### Appointment order features (v2)

| Column | Type | Nulls | คำอธิบาย |
|--------|------|-------|----------|
| `appointment_rank_in_day` | Int64 (nullable) | null เมื่อ license missing | ลำดับนัดของหมอในวันนั้น (1 = นัดแรก) |
| `is_first_case` | int | 0 | นัดแรกของวันหรือไม่ (0/1) — null license → 0 เสมอ |
...
Distribution เบ้ไปทางขวา — หัตถการส่วนใหญ่ 15-30 นาที แต่มี case ยาว ๆ ถึง 420 นาที
...
| ตัวเลข | ค่า |
|--------|-----|
| จำนวน rows | 252,359 (v2 เพิ่มขึ้นจาก 120K เพราะหยุด drop missing dentist) |
| จำนวน columns | 21 |
| คลินิก (unique) | 147 |
...
### Dentist Feature Strategy (ADR-001: Two-Tier Lookup)

`dentist_pseudo_id` เป็น **nullable** — ประมาณ 50% ของ rows ไม่มีค่า

```python
# Tier 1: มี dentist_pseudo_id → join กับ Doctor Profile Stats
# Tier 2: ไม่มี dentist → ใช้ clinic-level aggregate เป็น fallback
# Tier 3: ไม่มีทั้งคู่ → Global Median per treatment class

# ตัวอย่าง
doctor_stats = train_df[train_df["has_dentist_id"] == 1].groupby(
    "dentist_pseudo_id"
)["scheduled_duration_min"].agg(["mean", "count"])

# สำคัญ: fit lookup tables จาก train split เท่านั้น (ห้ามใช้ test)
```
...
4. **~50% ของ raw data ไม่มี license_no** — (ADR-001) ไม่ drop แล้ว เก็บไว้พร้อม `has_dentist_id=0` เพื่อให้ FE ใช้ Two-Tier Fallback strategy
...
- [ ] Shape = (252359, 21) ตรงกับ manifest.json
